...
float G_smithG_GGX(float NdotV, float alphaG) {
...
vec3 BRDF(vec3 n, vec3 v, vec3 l, float Kd, float Ks, float f0, float alpha) {
	...
	float G = G_smithG_GGX(NdotL, alpha);

	float fre_exp = pow(1.0 - LdotH, 5.0);
	float F = f0 + (1.0 - f0) * fre_exp;

	float val = Kd/PI + Ks * F * G * D / (4.0 * NdotV);
...
void main(void) {
	...
	if (t > 0.0) {
		...
		* BRDF(nor,-rd,sun_dir,0.0,0.6,ior,roughness) * sun_sha;
		//col += vec3(0.5, 0.7, 1.0)
		//* BRDF(nor,-rd,sky_dir,0.0,0.99,ior,roughness) * occ;
