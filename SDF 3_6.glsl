...
void main(void) {
	...
	if (t > 0.0) {
		...
		float fre_exp = pow(1.0 - dot(-rd, nor), 5.0);
		float f0 = ks;
		float fre = f0 + (1.0 - f0) * fre_exp;

		col += sun_spe * vec3(8.0, 6.0, 4.0) * sun_dif * sun_sha;
		col += sky_spe * vec3(0.5, 0.7, 1.0) * sky_dif * fre * occ;
		//coly_dif * min(occ, sun_sha) * fre;
		col += (1.0 - sky_spe) * vec3(0.3, 0.5, 0.2) * bou_dif * fre * occ;
