...
vec3 wave(vec2 dir, float s, float w , vec3 p) {
...
float map(vec3 p) {
	//vec3 q = vec3(p.x, p.y + fbm(p.xz) * 0.2, p.z);
	vec3 q = p + wave(vec2(1.0, 1.0), 0.5, 5.0, p)
		 + wave(vec2(0.0, 1.0), 0.25, 2.0, p);
		 //+ wave(vec2(-0.7, -1.0), 0.1, 0.1, p);
	return q.y + 1.25;
}
